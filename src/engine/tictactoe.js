/** Taken from https://github.com/TheRayOfSeasons/Multi-Lingual-Tic-Tac-Toe/blob/master/javascript/core/tictactoe.js */
...
export class Board {
  ...
  draw(useTokens = true) {
    ...
    for (let i = 0; i < this.height; i++) {
      for (let j = 0; j < this.width; j++) {
        const token = useTokens ? this.field[i][j].token : ctr.toString();
        ctr += useTokens ? 0 : 1;
        printed.push(j === 0 ? ` ${token} | ` : token);

        if (j > 0 && j < this.width - 1) { printed.push(' | '); }
      }
  ...
  writeTurn(player, move) {
    ...
    for (let i = 0; i < this.width; i++) {
      for (let j = 0; j < this.height; j++) {
        if (parseInt(move, 10) === ctr) {
          const empty = this.field[i][j].isEmpty();
          if (empty) this.field[i][j].token = player.token;
          return empty;
        }
  ...
  checkWin() {
    let win = false;
    const isConsistent = (item, i, arr) => item.token === arr[0].token && !item.isEmpty();
